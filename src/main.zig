...
fn findGatewayPid(allocator: std.mem.Allocator, cfg: *const yc.config.Config) u32 {
    // Only Windows lacks std.posix.kill (pid_t is not an integer there, so the
    // @intCast below would be a compile error). macOS and Linux both run the
    // POSIX PID-file path; the /proc fallback self-guards for non-Linux.
...
fn terminateAndWait(pid: u32) bool {
    // std.posix.kill is unavailable only on Windows (pid_t is not an integer);
    // macOS and Linux both run it. comptime so the body is not analyzed on Windows.
...
fn runCron(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    const cron_usage = std.fmt.comptimePrint(
        ...
        \\
        \\Commands:
        \\  list [--limit N] [--all] [--json] [--skill <name>] [--channel <name>] [--account <id>]
        \\       [--to <id>] [--status <ok|error|paused>] [--match <substring>]
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        var list_filter = yc.cron.CronListFilter{};
        {
            ...
            while (li < sub_args.len) : (li += 1) {
                if (std.mem.eql(u8, sub_args[li], "--json")) {
                ...
                } else if (std.mem.eql(u8, sub_args[li], "--account") and li + 1 < sub_args.len) {
                    li += 1;
                    list_filter.account_id = sub_args[li];
                    ...
                    std.debug.print("Usage: nullclaw cron list [--limit N] [--all] [--json] [--skill <name>] [--channel <name>] [--account <id>] [--to <id>] [--status <ok|error|paused>] [--match <substring>]\n", .{});
                    std_compat.process.exit(1);
                }
            }
        }

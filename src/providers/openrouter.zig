...
pub const OpenRouterProvider = struct {
    ...
    pub fn chatWithHistory(
        ...
        const resp_body = root.curlPostTimed(allocator, BASE_URL, body, &.{ auth_hdr, referer_hdr, title_hdr }, 0) catch |err| return root.preserveCurlTransportError(err, error.OpenRouterApiError);
    ...
    fn chatImpl(
        ...
        const resp_body = root.curlPostTimed(allocator, BASE_URL, body, &.{ auth_hdr, referer_hdr, title_hdr }, request.timeout_secs) catch |err| return root.preserveCurlTransportError(err, error.OpenRouterApiError);
        defer allocator.free(resp_body);

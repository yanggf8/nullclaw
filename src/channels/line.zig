    ...
    while (i < g_reply_cache_count) : (i += 1) {
        ...
        if (std.mem.eql(u8, cached.target[0..cached.target_len], target)) {
            if (now - cached.received_at_ms <= REPLY_TOKEN_TTL_MS) {
                // Snapshot the length before swap-remove: removeReplyCacheEntryLocked
                // overwrites g_reply_cache[i] (which `cached` aliases) with the last
                // entry, so reading cached.token_len afterwards would yield the wrong
                // length.
                const token_len = cached.token_len;
                @memcpy(out[0..token_len], cached.token[0..token_len]);
                removeReplyCacheEntryLocked(i);
                return out[0..token_len];
            }
            removeReplyCacheEntryLocked(i);
            return null;
        }

...
| Command | Purpose |
|---|---|
| `nullclaw cron list [--json] [--limit N] [--all] [--skill <name>] [--channel <name>] [--account <id>] [--to <id>] [--status <ok\|error\|paused>] [--match <substring>]` | Weekly chronological fire-time table (human) or JSON array; filters are ANDed and `--all` shows all matching jobs without a limit |

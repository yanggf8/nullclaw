...
这个配置本身不会启用外部 LLM 调用；操作者仍然必须显式传入 `--llm-triage external`。单次运行可用 `--llm-provider`、`--llm-model`、`--llm-max-calls` 覆盖这些配置。

每次 `external` 请求都会追加到 `~/.nullclaw/audit-log.jsonl`，格式 `{"timestamp": ..., "envelope": ..., "verdict": ...}`。日志仅追加，便于事后核对实际发出的元数据。

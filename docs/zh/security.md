...
- **預設 fail-closed（破壞性變更）：** 空的或省略的 `allow_from` 現在會在每個頻道**拒絕所有入站發送者**。未填寫 `allow_from` 的頻道會保持靜默，直到你列出可信使用者 ID / OpenID。
- `allow_from: ["*"]`：允許所有來源（高風險，僅顯式確認後使用）；空清單已不再代表「關閉過濾」。
- 其他情況通常是精確匹配 allowlist，或該頻道自己的 group-policy 語義。
